...
class SQLiteDB {
	...
	required init(gid:String) {
		...
		print("Database path: \(path)", terminator: "")
		...
		if !(fm.fileExistsAtPath(path)) {
			...
			if resourcePath != nil {
                ...
                if error != nil{
                    print("SQLiteDB - failed to copy writable version of DB!", terminator: "")
                    print("Error - \(error!.localizedDescription)", terminator: "")
                    return
                }
//				do {
//					let theResult = fm.copyItemAtPath(from, toPath: path)//try
//				}catch NSError{
//					error = error1
...
//				}
		...
		if error != SQLITE_OK {
			...
			print("SQLiteDB - failed to open DB!", terminator: "")
			sqlite3_close(db)
		}
		fmt.dateFormat = "YYYY-MM-dd HH:mm:ss"
	}
	...
	private func closeDatabase() {
		if db != nil {
			...
			launchCount--
			print("SQLiteDB - Launch count \(launchCount)", terminator: "")
			var clean = false
			if launchCount < 0 {
				clean = true
				launchCount = 500
			...
			// Clean DB
			print("SQLiteDB - Optimize DB", terminator: "")
			let sql = "VACUUM; ANALYZE"
			if execute(sql) != SQLITE_OK {
				print("SQLiteDB - Error cleaning DB", terminator: "")
			}
			sqlite3_close(db)
		}
	}
...
//			println("SQLiteDB - Cleaned up column type: \(tmp)")
			if containsValueByArray(intTypes,tmp) {
				return SQLITE_INTEGER
			}
			if containsValueByArray(realTypes,tmp) {
				return SQLITE_FLOAT
			}
			if containsValueByArray(charTypes,tmp) {
				return SQLITE_TEXT
			}
			if containsValueByArray(blobTypes,tmp) {
				return SQLITE_BLOB
			}
			if containsValueByArray(nullTypes,tmp) {
				return SQLITE_NULL
			}
			if containsValueByArray(dateTypes,tmp) {
				return SQLITE_DATE
			}
			return SQLITE_TEXT
		} else {
    ...
    private func containsValueByArray(source:[String],_ tmp:String)->Bool{
        for value in source{
            if value == tmp{
                return true
            }
